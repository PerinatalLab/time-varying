...
library(pammtools)
library(cowplot)
knitr::opts_knit$set(root.dir='/mnt/HARVEST')
knitr::opts_chunk$set(fig.width=7, fig.height=4) 
```

# Report on SNP `r params$i`

## Data

```{r genodata, cache=T}
# TODO update numbers for the new pheno file
# TODO include batch covariate

# read in filtered phenotypes
mfr_mid = read.table("ga_cleaned.csv", h=T, sep=";")
nrow(mfr_mid)  # 79194

# read in genotypes
gt = read.table("top1-moms-dosage.csv.gz", h=T, sep=" ")
gtinfo = gt[,1:6]
gt = t(gt[,7:ncol(gt)])
gt = data.frame(gt)
...
## Select and prepare the marker

```{r definesnp, cache=T}
# TODO temp
merged$GT = merged[,paste0("X",params$i)]
...
mod.pc = bam(ped_status ~ s(tend,bs='cr',k=11) + GT, data=ped, offset=offset, family=poisson())
summary(mod.pc)
...
```{r pammtvshape, cache=T, fig.height=3}
# time-varying by prespecified shape
mod.tv.pc.i = bam(ped_status ~ s(tend,bs='cr',k=11) + GT + GT:tend, data=ped, offset=offset, family=poisson())
summary(mod.tv.pc.i)
...
```{r pammsmooth, cache=T, fig.width=7, fig.height=6}
...
# VIZ
par(mfrow=c(1,2))
vis.gam(mod.tv.pc.i, theta=40)
vis.gam(mod.tv.pc.sm, theta=40)
vis.gam(mod.tv.pc.sm2, theta=40)
vis.gam(mod.tv.pc.sm2.r, theta=40)
```

```{r pammplots2, cache=T, fig.height=3}
