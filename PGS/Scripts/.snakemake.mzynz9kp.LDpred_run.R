...
snakemake <- Snakemake(
    input = list('Data/GA_pheno.txt', '/home/christopher.flatley/work2/chrisf/HapMap/EUR_hapmap_freq_allchr.txt', 'Data/Maternal_nodups_GAraw.txt', 'Data/Filtered_Merged/GA_genotypes.bed'),
    output = list('Results/info_snp', 'Results/fam.order', 'Results/h2_est', 'Results/beta_grid', 'Results/grid.param', 'Results/pred_grid', 'Results/result_grid', 'Results/grid_prs', 'Results/r2_matrix', 'Results/map', 'Data/Filtered_Merged/training_genotypes.bk'),
    params = list('Data/Filtered_Merged/GA_genotypes.rds'),
    wildcards = list(),
    threads = 20,
    log = list(),
    resources = list(),
...
pheno <- read.table(file = snakemake@input[[1]],  sep="\t", header = TRUE) ### CHANGE TO READ.TABLE
...
info <- read.table(file = snakemake@input[[2]],  sep="\t", header = TRUE) # CHANGE TO READ.TABLE
info = as.data.table(info)

#### Load and transform the summary stats

sumstats <- read.table(file = snakemake@input[[3]],  sep="\t", header = TRUE) # CHANGE TO READ.TABLE
...
###### removed , . after "GA~ Batch +"
null.model <-  paste0("GA ~ BATCH") %>%
    as.formula %>%
    lm(., data = y) %>%
    summary
...
reg.formula <- paste0("GA ~ PRS + BATCH", .) %>%
    as.formula
    reg.dat <- y
    max.r2 <- 0
